# astrbot_plugin_meme_manager_plus v3.9.1
...
> 穿搭由 `life_scheduler` 提供日程穿搭，情景适配 LLM 根据对话上下文临时调整（如"一起泡澡吧"→浴巾、"去运动"→运动服），且可设置上次输出的穿着细节（颜色、材质）会自动传递给下一次判断，保持连贯。强烈建议开启穿搭tag，体验提升不少。
...
默认参数是艾莉丝的调好的参数，提供novelai官网的key即可使用，可以先试试;
...
5. 发送消息触发 Bot 回复，插件会自动分析情绪 → 按表达欲望评分决定是否发图；每次决定发图后，概率判断是否触发LLM生图：触发则优先等待生图完成后发送生成图，生图失败时降级随机抽取发送；未触发则直接随机抽取发送；
...
2. 第二级（生图概率）：独立判定是否调用 API 生成新图；命中时优先发送生成图，生图失败降级随机抽取；未命中时直接随机抽取
...
第一级：score >= expression_threshold ?
    ↓ 通过
第二级：LLM 生图概率判定
    ├─ 命中 + 图库未满 → 调用 Gemini/Grok/GPT-Image-2 API 生图
    │       ├─ 成功 → 发送生成图 → 保存到图库
    │       └─ 失败 → 从该心情目录随机抽取一张发送
    └─ 未命中或图库已满 → 从该心情目录随机抽取一张发送
```
...
- **生图优先发送**：概率命中时优先等待 LLM 生图完成后发送生成图，生图失败时降级随机抽取；未命中时直接随机抽取
...
- **生图优先发送**：心情表情模式改为概率命中时优先等待 LLM 生图完成后发送生成图，生图失败时降级随机抽取；未命中时直接随机抽取。原"先发后生"逻辑（随机图立即发送 + 生图仅入库）已移除
