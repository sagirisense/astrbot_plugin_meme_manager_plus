# astrbot_plugin_meme_manager_plus v3.6.0
...
- **标签历史上下文**：LLM 补全标签时可参考最近 N 次生图标签，保持画面风格连贯过渡
- **标签历史持久化**：按会话隔离的标签历史缓存到 `tag_cache/` 目录，重启后自动恢复
- **穿搭 tag 注入**：从 life_scheduler 插件获取今日穿搭，LLM 转换为 NovelAI tag 后注入
- **权重控制**：标签历史和穿搭 tag 均支持独立的 NovelAI 权重参数
...
5. 发送消息触发 Bot 回复，插件会自动分析情绪 → 按表达欲望评分决定是否发图
...
5. 发送消息触发 Bot 回复，插件会按概率自动生图并发送

**Opus 会员免费生图**：默认配置（V4.5 Full, 832×1216, 28 步, 纯文生图）在 Opus 会员（$25/月）下免费无限使用。
...
开启后替代心情表情流程。Opus 会员使用默认配置可**免费无限生图**。
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| novelai_enabled | 启用 NovelAI 模式 | false |
...
| novelai_tag_history_size | 标签历史上下文（LLM 参考最近 N 次标签，0=关闭） | 0 |
| novelai_history_weight | 标签历史权重（最新条目权重，最旧固定 0.3，线性衰减） | 0.8 |
| novelai_use_outfit | 注入穿搭 tag（从 life_scheduler 获取穿搭） | false |
| novelai_outfit_weight | 穿搭 tag 权重（1.0=不降权，0.85=轻微降权） | 0.85 |
...
| `ni重置` | 清空 NovelAI 标签历史和穿搭缓存（含持久化文件） |
| `穿搭` | 查看穿搭/标签历史诊断信息 |
| `穿搭 1` | 开启穿搭 tag 注入 |
...
- **穿搭 tag 权重**：可配置穿搭 tag 的 NovelAI 权重（默认 0.85），用 `(tag:weight)` 语法精确控制
